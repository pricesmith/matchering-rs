...
fn __check_sample_rate(
    ...
    if sample_rate == required_sample_rate {
        // [todo] add - logging messages
                                ...
                                // (array, sample_rate, required_sample_rate, axis=0)
        array = samplerate::convert(
            sample_rate: u32, 
            required_sample_rate: u32, 
            channels: usize, 
...
// not super important
fn __check_clipping_limiting(
    input: ndarray::Array,
    clipping_samples_threshold: i32,
    limited_samples_threshold: i32,
    // warning_code_clipping: Code,
    // warning_code_limiting: Code,
) {
    let max_value, max_count = count_max_peaks(input);

    if max_count > clipping_samples_threshold {
        if np.isclose(max_value, 1.0) {
            warning(warning_code_clipping);
        }
    } else if max_count > limited_samples_threshold {

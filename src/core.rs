...
fn process(
    ...
    if (
        ...
        or not (dsp::channel_count(array: &target) == dsp::channel_count(array: &reference) == 2)
        or not (dsp::size(array: &target) > config.fft_size and dsp::size(array: &reference) > config.fft_size)
    ) {
        raise ModuleError(Code.ERROR_VALIDATION);
    }

    //  Process
    result, result_no_limiter, result_no_limiter_normalized = main( // call "main" "process"??
        target,
        reference,
        config,
    ...
    if (preview_target || preview_result) {
        result = next(
            item
